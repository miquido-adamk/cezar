...
| `CEZ_LOOPS=1` | Turn on **task loops**: the Loops view and the composer's Loop mode appear, and a loop runs an ordered list of items as separate agent sessions — one at a time, each in its own worktree — advancing only when the previous one finishes. Off by default and only the exact value `1` enables it; without it the loops endpoints answer `409` and the nav item is absent. Read at boot, so restart after changing it. Starting a loop can spawn many unattended, paid agent sessions, which is why it is opt-in. Definitions, cursors and receipts are retained, so flipping it off and on again loses nothing. |
| `CEZ_LOOP_AUTO_MERGE=1` | **Dangerous escape hatch:** allow a task loop to be given the `merge` landing policy, which opens a pull request per item and **merges it** once it is genuinely mergeable — so the next item starts from a base containing the previous one. This reverses cezar's “ends at a review gate (never auto-merges)” invariant and lands code with no human reviewing it; a red or unmergeable PR is left open and the loop continues. Requires `CEZ_LOOPS=1` as well: two locks, because per-loop opt-in alone would put an auto-merge one dropdown away from any cockpit user. Off by default; only the exact value `1` enables it. Enforced server-side — the loops routes answer `409` for `merge` without it — and again at merge time, so a loop configured while it was on stops merging when it is turned off. |
